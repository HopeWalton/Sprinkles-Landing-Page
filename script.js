...
// Dev Shop code
function featuredCupcake(img_id) {
  let img = document.getElementById(img_id);
  let name = document.getElementById("featured-cupcake-name");

  let flavorObject = findFlavorObject();
  let flavorName = flavorObject['name'];
  let flavorImg = flavorObject['img'];

  img.src = flavorImg;
  name.innerHTML = flavorName;
}